...
double trigonom;                       // переменная для перевода из радиан в градусы
...
void MainWindow::unary_operation_pressed() 
{
    ...
    {
        labelNumber = ui->label->text().toDouble();
        trigonom = ui->label->text().toDouble();            // помещаем число, чтобы потом можно было использовать его в переводе из радиан в градусы
    ...
    {
        labelNumber = ui->label->text().toDouble();
        trigonom = ui->label->text().toDouble();
...
void MainWindow::on_pushButton_deg_clicked()
{
    ...
    if (ui->top_label->text().contains("sin")) {
        ...
        if ((std::remainder(trigonom, 90)) == 0) {

            labelNumber = trigonom / 90;        // получаем целую часть от деления числа на 90
            ...
            /* Иначе, если число не делится нацело на 90, то вычисляем синус в градусах по формуле */
                labelNumber = std::sin(trigonom * PI / 180);
    ...
    if (ui->top_label->text().contains("cos")) {
        ...
        labelNumber = std::cos(trigonom * PI / 180);
    ...
    if (ui->top_label->text().contains("tan")) {
        ...
        if ((std::remainder(trigonom, 90)) == 0) {
            ...
            labelNumber = std::tan(trigonom * PI / 180);
